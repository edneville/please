...
use please::util::{UserData,read_config,can_edit,get_editor,challenge_password};
use chrono::Utc;
...
use nix::unistd::{fork,setuid,setgid,setgroups,gethostname,ForkResult};
...
fn setup_temp_edit_file( service: &str, source_file: &Path, original_uid: u32, original_gid: u32, original_user: &str ) -> String {

    let tmp_edit_file = format!( "/tmp/{}.{}.{}", source_file.file_name().unwrap().to_str().unwrap(), service, original_user );
    ...
    nix::unistd::chown( tmp_edit_file_path,
        Some( nix::unistd::Uid::from_raw( original_uid ) ),
        Some( nix::unistd::Gid::from_raw( original_gid ) )
    ).unwrap();

    nix::sys::stat::fchmodat( None, tmp_edit_file_path,
        nix::sys::stat::Mode::S_IRUSR | nix::sys::stat::Mode::S_IWUSR ,
        nix::sys::stat::FchmodatFlags::FollowSymlink).unwrap();
...
fn main() {
    ...
    match &entry {
        Err(_) => {
            println!( "You may not edit \"{}\" on {} as {}", new_args.join( " " ), &hostname, &target );
            return;
        }
        Ok(x) => {
            if !x.permit {
